...
function generateObjectPropertyGetter(
  ...
  if (field.type.isRootObject) {
    ...
    writer.write(`const session = VossSession.getSessionOf(this);\n`);
...
function generateObjectPropertySetter(
  ...
  if (object.isRoot) {
    writer.write('set' + toPascalCase(field.name) + '() {\n');
    writer.write(`throw new Error('Not Implemented.');\n`);
    writer.write('}\n');
    return;
  }

  const newValueType = field.type.isPrimitive
...
function generateObjectPatch(writer: PrettyWriter, object: IRObject): void {
  const dataInterfaceName = toPascalCase(object.name) + '$Data';
  writer.write(`patch(patch: Partial<${dataInterfaceName}>): void {\n`);
